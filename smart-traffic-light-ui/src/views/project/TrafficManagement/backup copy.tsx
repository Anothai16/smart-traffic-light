...
const TrafficManagement = () => {
    ...
    const handleUpdateMode = async () => {
        ...
        // ✅ แยกการทำงานออกจาก try-catch
        if (response.data?.success) {
            setCurrentMode(selectedMode);
            messageApi.success(response.data.message || `Successfully changed mode to ${selectedMode}!`);
        } else {
            messageApi.error(response.data.message || `Failed to change mode.`);
        }

    } catch (error) {
        const err = error as AxiosError<ApiErrorResponse>;
        const errorMessage = err.response?.data?.message || err.message || 'An unexpected error occurred';
        messageApi.error(`Failed to change mode. Error: ${errorMessage}`);
    } finally {
        setLoading(false);
    }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.log('API Response:', response.data); // ✅ เพิ่มส่วนนี้เพื่อตรวจสอบการตอบกลับของ API
            
            if (response.data?.success) {
                messageApi.success(response.data.message || 'Successfully changed!');
            } else {
                messageApi.error(response.data.message || 'Failed to change intersection times.');
            }

        } catch (error: any) {
            const err = error as AxiosError<ApiErrorResponse>;
            const errorMessage = err.response?.data?.message || err.message || 'An unexpected error occurred';
            console.error('API Error:', errorMessage); // ✅ เพิ่มส่วนนี้เพื่อตรวจสอบข้อผิดพลาด
            messageApi.error(`An error occurred: ${errorMessage}`);
        } finally {
            setLoading(false);
        }
    };
